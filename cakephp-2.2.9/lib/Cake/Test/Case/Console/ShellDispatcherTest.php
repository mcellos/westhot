 ...
 */
	public function testParseParams() {
		...
		$params = array(
			...
			'new',
			'-working',
			'/usr/share/nginx/html/htdocs'
		);
		$expected = array(
			'app' => 'new',
			'webroot' => 'webroot',
			'working' => str_replace('/', DS, '/usr/share/nginx/html/htdocs/new'),
			'root' => str_replace('/', DS,'/usr/share/nginx/html/htdocs')
		);
		...
		$expected = array(
			'app' => 'old',
			'webroot' => 'webroot',
			'working' => str_replace('/', DS, '/usr/share/nginx/html/htdocs/old'),
			'root' => str_replace('/', DS,'/usr/share/nginx/html/htdocs')
		);
		$Dispatcher->parseParams($params);
		$this->assertEquals($expected, $Dispatcher->params);
